...
formulario.addEventListener("submit", (e) => {
	...
	return inputUser, inputConsulta; // para q me devuelva esos valores y yo poder guardar en localStorage'?
});
/* console.log(inputUser);
console.log(inputConsulta);
 */
// guardar consulta-- o Carro de compras
const inputUsuario = document.querySelector(".contacto_input");
const inputConsultas = document.querySelector(".consulta_text");
...
const productoSeleccionado = productos.findIndex((el) => { /* el) => el.id == id */
	return el.id === productos[id].id;  
});
if (productoSeleccionado === -1) {
	const productoAgregado = productos[id];
	carritoCompra.push(productoAgregado);
} else {
	carritoCompra[productoSeleccionado].cantidad += 1;
}